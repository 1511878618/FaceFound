...
class MaskedAutoencoderSwin(nn.Module):
    ...
    def recover(self, imgs, masks, preds):
        # Reassemble predictions into images
        imgs = self.patchify(imgs, self.stride)  # [N,L,-1]
        if self.norm_pix_loss:
            mean = imgs.mean(dim=-1, keepdim=True)
            var = imgs.var(dim=-1, keepdim=True)
            preds = preds * (var + 1.e-6)**.5 + mean
        pred_imgs = []
        for img, pred, mask in zip(imgs, preds, masks):
            mask_num = mask.sum()
            pred_img = torch.zeros_like(img)
            pred_img[~mask] = img[~mask]
            pred_img[mask] = pred[-mask_num:]
            pred_imgs.append(pred_img)
        pred_imgs = torch.stack(pred_imgs)  # [N, L, -1]
        pred_imgs = self.unpatchify(pred_imgs, self.stride)
        # Denormalize
        device = pred_imgs.device
        mean = torch.tensor([0.485, 0.456, 0.406]).view(1,-1,1,1).to(device)
        std = torch.tensor([0.229, 0.224, 0.225]).view(1,-1,1,1).to(device)
        pred_imgs.mul_(std).add_(mean)
        return pred_imgs

    def forward_encoder(self, x, mask):
        N = x.size(0)
        # embed patches
        x = self.patch_embed(x)  # [N,3,H,W] -> [N,embed_h*embed_w,embed_dim] embed_h=embed_w=img_size//patch_size=num_patches**0.5

        # secondary mask 
        L = mask.size(1)  # clips
        vis_cnt = L - len(mask[0].nonzero())  # Number of visible clips (0.25*clips)
        vis_final_cnt = int(vis_cnt * (1. - self.vis_mask_ratio))  # Final visible clips (0.25*clips*0.75)
        ...
        M = int(L ** 0.5)
        scale = self.embed_h // M  # Patch to clip scaling relationship
        mask = mask.reshape(N, M, M)
        mask = mask.repeat_interleave(scale, 1).repeat_interleave(scale, 2).unsqueeze(1).contiguous()  # (N, 1, embed_h, embed_w)
        new_mask = new_mask.reshape(N, M, M)
        new_mask = new_mask.repeat_interleave(scale, 1).repeat_interleave(scale, 2).unsqueeze(1).contiguous()  # (N, 1, embed_h, embed_w)

        # add vis_mask_token
        if hasattr(self, 'vis_mask_token'):
            token_mask = (~mask).int() - (~new_mask).int()  # (N, 1, embed_h, embed_w)
            vis_mask_token = self.patch_embed.norm(self.vis_mask_token)  # [1,1,embed_dim]
            ...
            vis_mask_token = vis_mask_token * token_mask  # (N, embed_dim, embed_h, embed_w)
        else:
            vis_mask_token = 0

        # prepare variables
        K = self.kernel_stride  # stride // patch_size
        H, W = self.embed_h, self.embed_w  # img_size // patch_size
        self.kernel = self.kernel.to(x.device)

        # x to image shape (N, L, D) -> (N, C, H//2, W//2)
        x = x.reshape(N, self.embed_h, self.embed_w, self.embed_dim).permute(0, 3, 1, 2)  # (N, embed_dim, embed_h, embed_w)
        x = x * (~new_mask) + vis_mask_token  # (N, embed_dim, embed_h, embed_w)
        x = rearrange(x, 'b c (h p1) (w p2) -> (b h w) c p1 p2', p1=K*2, p2=K*2)  # [N*clip_h*clip_w, embed_dim, K*2, K*2] [N*clip_h*clip_w, embed_dim, embed_h//clip_h, embed_w//clip_w]
        x = F.conv2d(x, self.kernel, dilation=K, groups=self.embed_dim)  # (N*clip_h*clip_w, embed_dim, K, K)
        x = rearrange(x, '(b h w) c p1 p2 -> b c (h p1) (w p2)', h=H//(K*2), w=W//(K*2))  # [N, embed_dim, embed_h//2, embed_w//2]
        ...
        pos_hw = pos_hw.permute(0, 2, 3, 1) # (N, embed_h//2, embed_w//2, 2)

        x = x.permute(0, 2, 3, 1).reshape(N, -1, self.embed_dim)  # [N, embed_dim, clip_h * K // 2, clip_w * K // 2] -> [N, embed_h*embed_w//4, embed_dim]

        # apply Transformer blocks
        for blk in self.blocks:
            x, pos_hw = blk(x, pos_hw)
        x = self.norm(x)  # [N, [0.5*embed_h*(0.5**3)]*[0.5*embed_w*(0.5**3)], embed_dim*(2**3)]

        return x

    def forward_decoder(self, x, mask):
        # embed tokens
        x = self.decoder_embed(x)  # [N, [0.5*embed_h*(0.5**3)]*[0.5*embed_w*(0.5**3)], embed_dim*(2**3)] -> (N, L, 4*decoder_embed_dim)  L=[0.5*embed_h*(0.5**3)]*[0.5*embed_w*(0.5**3)]
        x_vis = x
        N, L, nD = x_vis.shape  # L=[0.5*embed_h*(0.5**3)]*[0.5*embed_w*(0.5**3)], nD=4*decoder_embed_dim
        M = int(L**0.5)  # M=0.5*embed_h*(0.5**3)=0.5*embed_w*(0.5**3)
        x_vis = self.decoder_expand(x_vis.permute(0, 2, 1).reshape(-1, nD, M, M)).flatten(2)  # [N,L,nD]->[N,nD,M,M]->[N,nD*0.5*0.5,(M*2)*(M*2)]=[N,decoder_embed_dim,(M*2)*(M*2)]
        x_vis = x_vis.permute(0, 2, 1)  # [N,nD*0.5*0.5,(M*2)*(M*2)]->[N,(M*2)*(M*2),nD*0.5*0.5]
        _, _, D = x_vis.shape  # D=decoder_embed_dim

        # append mask tokens to sequence
        expand_pos_embed = self.decoder_pos_embed.expand(N, -1, -1)  # [N,decoder_num_patches,decoder_embed_dim]
        pos_vis = expand_pos_embed[~mask].reshape(N, -1, D)  # [N,decoder_num_patches*0.25,decoder_embed_dim]
        pos_mask = expand_pos_embed[mask].reshape(N, -1, D)  # [N,decoder_num_patches*0.75,decoder_embed_dim]

        x = torch.cat([x_vis + pos_vis, self.mask_token + pos_mask], dim=1)  # assert (M*2)*(M*2) == decoder_num_patches*0.25 --> stride == 4 * patch_size
    ...
    def forward(self, imgs, mask):
        """
        imgs: [N, 3, H, W]
        mask: [N, clips], 0 is keep, 1 is remove, clips = H*W // stride**2
        """
        latent = self.forward_encoder(imgs, mask) # returned mask may change [N,64,1536] # [N, [0.5*embed_h*(0.5**3)]*[0.5*embed_w*(0.5**3)], embed_dim*(2**3)]
...
def mae_swin_large_256d16(**kwargs):
    model = MaskedAutoencoderSwin(
        img_size=256, patch_size=4, in_chans=3, stride=16,
        embed_dim=192, depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48],
        posmlp_dim=64, mlp_ratio=4, window_size=16,
...
def mae_swin_large_512d16_dec512d16(**kwargs):
    model = MaskedAutoencoderSwin(
...
def mae_swin_large_512d16_dec1024d16(**kwargs):
    model = MaskedAutoencoderSwin(
        img_size=512, patch_size=4, in_chans=3, stride=16,
        embed_dim=192, depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48],
        posmlp_dim=64, mlp_ratio=4, window_size=16, # 16 for finetune
...
def mae_swin_large__512d16__tokensize16(**kwargs):
    model = MaskedAutoencoderSwin(
        img_size=512, patch_size=4, in_chans=3, stride=32,  # Number of blocks
        embed_dim=192, depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48],
        posmlp_dim=64, mlp_ratio=4, window_size=16, # 16 for finetune
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=32,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
mae_swin_large_256d16 = mae_swin_large_256d16
mae_swin_large_512 = mae_swin_large_512d16_dec512d16
mae_swin_large_512_large_decoder = mae_swin_large_512d16_dec1024d16 # larger decoder: 1024 dim, 16 blocks
mae_swin_large_512_tokensize16 = mae_swin_large__512d16__tokensize16
