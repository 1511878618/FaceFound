...
from utils.models_mae_swin import SwinBlock, PatchEmbed, PatchMerge
...
def swin_large_512to256(**kwargs):
    model = Swin(img_size=256, patch_size=4, in_chans=3, 
                 embed_dim=192, depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48],
                 posmlp_dim=64, mlp_ratio=4, window_size=8, # Pretrain uses 512 16
                 norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs
    )
    return model

def swin_large_512(**kwargs):
    model = Swin(
        img_size=512, patch_size=4, in_chans=3, embed_dim=192, depths=[2, 2, 18, 2], 
        num_heads=[6, 12, 24, 48], posmlp_dim=64, mlp_ratio=4, window_size=16, 
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs
    )
    return model
